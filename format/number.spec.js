...
import { bigIntToBinUint256BEClamped, bigIntToBinUint64LE, bigIntToBinUint64LEClamped, bigIntToBinUintLE, bigIntToCompactSize, binToBigIntUint256BE, binToBigIntUint64LE, binToBigIntUintBE, binToBigIntUintLE, binToHex, binToNumberInt16LE, binToNumberInt32LE, binToNumberUint16LE, binToNumberUint32LE, binToNumberUintLE, CompactSizeError, compactSizePrefixToSize, compactSizeToBigInt, hexToBin, int32SignedToUnsigned, int32UnsignedToSigned, numberToBinInt16LE, numberToBinInt32LE, numberToBinInt32TwosCompliment, numberToBinUint16BE, numberToBinUint16LE, numberToBinUint16LEClamped, numberToBinUint32BE, numberToBinUint32LE, numberToBinUint32LEClamped, numberToBinUintLE, readCompactSize, readCompactSizeMinimal, } from '../lib.js';
...
test('bigIntToCompactSize: larger values return modulo result after opcode', (t) => {
    t.deepEqual(bigIntToCompactSize(0x010000000000000001n), Uint8Array.from([0xff, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]));
});
...
test('readCompactSize', (t) => {
    ...
    t.deepEqual(readCompactSize({ bin: Uint8Array.from([253, 0, 0]), index: 0 }), { position: { bin: Uint8Array.from([253, 0, 0]), index: 3 }, result: 0n });
    t.deepEqual(readCompactSize({ bin: Uint8Array.from([254, 0, 0, 0, 0]), index: 0 }), {
        position: { bin: Uint8Array.from([254, 0, 0, 0, 0]), index: 5 },
        result: 0n,
    });
    t.deepEqual(readCompactSize({
    ...
    t.deepEqual(readCompactSize({ bin: Uint8Array.from([253, 253, 0]), index: 0 }), {
        position: { bin: Uint8Array.from([253, 253, 0]), index: 3 },
        result: 253n,
    });
    t.deepEqual(readCompactSize({ bin: Uint8Array.from([]), index: 0 }), CompactSizeError.noPrefix);
});
test('readCompactSizeMinimal', (t) => {
    ...
    t.deepEqual(readCompactSizeMinimal({ bin: Uint8Array.from([253, 1, 0]), index: 0 }), `${CompactSizeError.nonMinimal} Value: 1, encoded length: 3, canonical length: 1`);
});
test('compactSizeToBigInt', (t) => {
    ...
    t.deepEqual(compactSizeToBigInt(Uint8Array.from([253])), 'Error reading CompactSize: insufficient bytes. CompactSize prefix 253 requires at least 3 bytes. Remaining bytes: 1');
    t.deepEqual(compactSizeToBigInt(Uint8Array.from([253, 0, 254, 0])), 'Error decoding CompactSize: unexpected bytes after CompactSize. CompactSize ends at index 3, but input includes 4 bytes.');
});
...
test(compactSizeVector, '0000ff0000000001000000', 0x0100000000n, 11, 2, 'ff0000000001000000');
...
testProp('[fast-check] bigIntToCompactSize <-> compactSizeToBigInt', [fc.bigUintN(64)], (t, uint64) => {
